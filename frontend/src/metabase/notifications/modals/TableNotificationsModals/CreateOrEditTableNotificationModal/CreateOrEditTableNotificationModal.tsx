...
// Hook to fetch necessary data for the modal
...
// Hook to manage the form state (requestBody)
  ...
  useEffect(() => {
    ...
    if (!requestBody && user && channelSpec && hookChannels) {
      const eventName: NotificationTriggerEvent =
        notification?.payload.event_name ?? "event/rows.created";
...
// Hook to manage the preview panel state and data fetching
...
// Hook to manage saving/updating and the confirmation modal
const useNotificationSave = (
  ...
  const onCreateOrEditAlert = useCallback(async () => {
    ...
    try {
      ...
      //   addUndo({
      //     message: isEditMode
      //       ? t`Failed to update notification`
      //       : t`Failed to create notification`,
      //   }),
      // );
    }
  }, [
